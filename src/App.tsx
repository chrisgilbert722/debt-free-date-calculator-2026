...
import { calculateDebtFreeDate } from './logic/debtFreeCalculations';
import type { DebtFreeInput } from './logic/debtFreeCalculations';

function App() {
  const [values, setValues] = useState<DebtFreeInput>({
    totalDebt: 15000,
    monthlyPayment: 400,
    interestRate: 18
  });

  const handleChange = (field: keyof DebtFreeInput, value: number | boolean) => {
    setValues(prev => ({ ...prev, [field]: value as number }));
  };

  const result = calculateDebtFreeDate(values);
